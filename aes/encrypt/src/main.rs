...
let ciphertext = b"\xa6\xad\x97\x4d\x5c\xea\x1d\x36\xd2\xf3\x67\x98\x09\x07\xed\x32";
println!("plaintext is {:?}", key);
let mut iv = *b"\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F\
 ...
 aes_ige(ciphertext, &mut output, &key, &mut iv, Mode::Decrypt);
 println!("{:?}", output);
